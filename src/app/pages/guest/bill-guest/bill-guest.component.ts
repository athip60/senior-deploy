...
@Component({
  selector: 'app-bill-guest',
  templateUrl: './bill-guest.component.html',
  styleUrls: ['./bill-guest.component.scss']
})
export class BillGuestComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.billService.findGuest(user.id, this.token).subscribe(dataBills => {
      ...
      this.totalUnit += dataBills[0].total_unit
    })
  }
  ...
  makePdfBill(data, option = 'open') {
    ...
    const pdf_file = {
      ...
      content: [
        ...
        {
          stack: [
            { text: `วันที่ : ${this.datepipe.transform(data.createdAt, 'd MMMM y')}` },
      ...
      if (res === "true") {
        this.billService.updateBillGuest(data.id, { bill_status: 'รอการตรวจสอบจากเจ้าของหอพัก' }, this.token).subscribe((afterSend) => {
          if (afterSend.status === true) {
            this.authService.updateUser({ status_bill: 'รอการตรวจสอบจากเจ้าของหอพัก' }, data.user_in_room, this.token).subscribe((afterUser) => {
              let dataNotify = {
                user_id: data.user_issur_id,
                description: "มีการส่งหลักฐานการโอนเงินกลับมา",
                status: "unread",
              }
